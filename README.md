...
> **Status:** v0.15.1 keeps the full Vision request surface, adds explicit `VNRequest` / request-handler / video-processor wrappers, and ships an audited request/observation coverage matrix (`COVERAGE.md`) plus a gold-standard multi-file Swift bridge.
...
All request-type modules can be enabled independently, and the default feature set still enables the full Vision surface. v0.15.1 adds both the explicit request-processing wrappers and an audited `COVERAGE.md` matrix for every current request + observation type in the macOS SDK.
...
- [x] Explicit `VNRequest` / `VNObservation` / request-handler / `VNVideoProcessor` wrappers for OCR pipelines
